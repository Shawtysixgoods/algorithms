...
Выбери опорный элемент.
Раздели остальные на "меньше", "равно", "больше".
Повтори то же самое для группы "меньше" и группы "больше" (это называется рекурсия – функция вызывает сама себя для задач поменьше).
Собери результаты: [отсортированные меньшие] + [равные] + [отсортированные большие].
Ключевая идея – рекурсия и разделение задачи на подзадачи. Это делает её быстрой для большинства случаев, но выбор "опорного" элемента может сильно влиять на скорость.
